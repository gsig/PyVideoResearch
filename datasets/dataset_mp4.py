...
class DatasetMP4(DatasetJPG):
    def __init__(self, *args, **kwargs):
        if 'train_gap' not in kwargs:
            kwargs['train_gap'] = 64
        if 'test_gap' not in kwargs:
            kwargs['test_gap'] = 10
        super(DatasetMP4, self).__init__(*args, **kwargs)

    def get_video_basedir(self, path, vid, label):
    ...
    def _prepare(self, path, labels, split):
        ...
        for i, (k, label) in enumerate(labels.items()):
            vid = label['vid']
            iddir = self.get_video_basedir(path, vid, label)
            if i % 1000 == 0:
