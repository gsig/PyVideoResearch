...
class CharadesSync2(Charades):
    def __init__(self, *args, **kwargs):
        self.testGAP = 64
        super(CharadesSync2, self).__init__(*args, **kwargs)
        ...
        for i, (vid, label) in enumerate(labels.iteritems()):
            ...
            if split == 'val_video':
                ...
                if n <= GAP*testGAP+1:
                    ...
                    shift = np.random.randint(n-GAP*testGAP-1)
                    spacing = np.arange(shift, shift+GAP*testGAP, GAP)
                for loc in spacing:
...
def get(args):
    ...
    valvideo_dataset = CharadesSync2(
