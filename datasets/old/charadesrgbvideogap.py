...
class CharadesVideoGAP(Charades):
    def __init__(self, *args, **kwargs):
        self.trainGAP = 64
        self.testGAP = 25
        self.GAP = 4
        self.inputsize = 224
        super(CharadesVideoGAP, self).__init__(*args, **kwargs)
    ...
    def prepare(self, path, labels, split):
        ...
        for i, (vid, label) in enumerate(labels.iteritems()):
            ...
            data['id'] = vid
            if split == 'val_video':
                spacing = np.linspace(0, n-self.GAP*self.trainGAP-1, self.testGAP)
                for loc in spacing:
                    data['shift'] = loc
                    datas.append(data)
            else:
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if hasattr(self.data['datas'][index], 'shift'):
                ...
                shift = np.random.randint(n-self.GAP*self.trainGAP-2)

            resize = transforms.Resize(int(256./224*self.inputsize))
            spacing = np.arange(shift, shift+self.GAP*self.trainGAP, self.GAP)
            for loc in spacing:
                ii = int(np.floor(loc))
